...
class TestDiffRootDynamicsTDWI:
    def test_gradients_tdwi_lai_root_dynamics(self):
    ...
    def test_gradients_tdwi_lai_root_dynamics_numerical(self):
        model = get_test_diff_root_model()
    ...
    def test_gradients_tdwi_twlv_root_dynamics(self):
        # prepare model input
        model = get_test_diff_root_model()
        tdwi = torch.nn.Parameter(torch.tensor(0.2, dtype=torch.float32))
        output = model({"TDWI": tdwi})
...
class TestDiffRootDynamicsSPAN:
    def test_gradients_span_lai_root_dynamics(self):
        # prepare model input
        model = get_test_diff_root_model()
    ...
    def test_gradients_span_lai_root_dynamics_numerical(self):
        model = get_test_diff_root_model()
    ...
    def test_gradients_span_twlv_root_dynamics(self):
        # prepare model input
        model = get_test_diff_root_model()
        span = torch.nn.Parameter(torch.tensor(30, dtype=torch.float32))
        output = model({"SPAN": span})
